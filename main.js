...
const dictPath = path.normalize(path.join(__dirname, 'resources', 'speciesDict.json'));

if (fs.existsSync(dictPath)) {
  const dict = JSON.parse(fs.readFileSync(dictPath, 'utf8'));
  speciesRepository.loadFromDict(dict);
  console.log('[main] species dict loaded:', Object.keys(dict.ebird_to_id ?? {}).length, 'eBird codes');
} else {
  console.warn('[main] resources/speciesDict.json not found — run scripts/convert_species_dict.py first.');
}
...
function createWindow() {
  ...
  mainWindow.loadFile(path.join(__dirname, 'src/presentation/renderer/index.html'));
